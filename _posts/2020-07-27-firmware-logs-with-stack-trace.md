...
<figure class="col-md-12">
  <img src="/img/posts/stack_trace_log/colorful_stack_unbundling.gif" alt="Colorful stack" class="img-responsive">
  <figcaption>Colorful stack unbundling</figcaption>
</figure>
...
We could implement stack tracing from an RTT Client but it might be more interesting to have GDB connected if your debugger is attached anyway. For a good starting point, [checkout that article](https://interrupt.memfault.com/blog/cortex-m-fault-debug).
...
Hopefully, we won't reinvent the wheel as [adamgreen](https://github.com/adamgreen) made those tools available on Github with [CrashCatcher](https://github.com/adamgreen/CrashCatcher) and [CrashDebug](https://github.com/adamgreen/CrashDebug). 🙏
...
To perform the stack dump, we are going to use CrashCatcher. If you want to read more about that tool, head on to the [git repo](https://github.com/adamgreen/CrashCatcher). Let's start.
...
As an example, I made [that file available on my Github](https://github.com/fouge/nrf_utils/blob/master/debug/log/crash_hexdump.c). Make sure to add yours to your Makefile before compiling your program. You don't need a header file as the signatures are taken from `CrashCatcher.h`. 
...
Make sure to get the CrashDebug executable [here](https://github.com/adamgreen/CrashDebug/tree/master/bins) or compile it yourself following the instructions from [the git repo](https://github.com/adamgreen/CrashDebug). The path to CrashDebug is hardcoded in the following example so you probably want to change it.
...
<pre>2020-07-26 11:08:13.040 [<font color="#268BD2">1595754491 </font><font color="#839496"><b>(-2.041s)</b></font>:<font color="#268BD2">0</font>:<font color="#2AA198">../../main.c</font>:<font color="#D33682">643</font>] ⚡ My awesome product
...
The Python script is not the only piece needed to print such a beautiful log obviously. On the target side, I have to send the information to print. Macros are used to make it easier to print *upgraded* messages. You will find those two files in [my Github repo](https://github.com/fouge/nrf_utils/tree/master/debug/log) if you want to use them.

---

Such stack tracing has long been implemented in higher-level programming languages but tools for Firmware development have always been lagging compared to most of the software industry. This post tries to bridge the gap by building our tools. It will now be a pleasure not to spend too much time on a bug. 😉
